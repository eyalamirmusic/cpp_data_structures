...
The INTERFACE target declares `target_compile_features(... INTERFACE cxx_std_20)`, so consumers automatically get a C++20-or-higher requirement propagated onto anything that links it.
...
- **`int` size semantics, not `size_t`.** The `Vector` / `Array` wrappers deliberately expose `int`-based sizes and indices — do not "fix" this to `size_t`. Helper methods (`contains`, `eraseIf`, `addIfNotThere`, `sort`, …) are the whole point of these wrappers over raw STL. `size()` asserts that the count fits an `int`; `getSize()` is the full `size_t`, for STL calls and for a `Span` over more than an `int` describes, and `Span` builds from a container's `getSize()` when it has one. `Span` wraps `std::span` the way `Vector` wraps `std::vector` — `getSpan()` beside `getVector()` / `getArray()`.
- **`Array` is constexpr throughout.** Every member except the two `sort` overloads (`std::sort` only becomes constexpr in C++26) is usable in a constant expression, as are the `Vectors::` search and reverse helpers they delegate to (`getIndexOf`, `getIndexIf`, `countIf`, `contains`, `find`, `findIf`, `reverse`, `rotate`). That is what lets a type that *stores* an `Array` — a matrix, a lookup table — keep its own constexpr, so don't add a non-constexpr member without a reason. Accessors need const overloads for the same reason: a `constexpr` object is a const one.
